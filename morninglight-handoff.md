...
- `data_term1.js`, `data_term2.js`, `data_term3.js` — already built. Each merges its lesson content into `window.DEVOTIONS["Term N"]` via:
  ...
  ```
- **Your job, most likely:** start Term 4 = Connect B2 Upper Primary, Lessons 11–20, continuing the alternating pattern. Terms 1–3 are all complete. See the scanned-manual gap warning below before planning how far you can get.
...
**SOURCE MATERIAL GAP — read before planning Term 4:** the scanned Connect B2 Upper Primary manual in `Connect Books Scanned/` covers **Lessons 1–13 and 18–20 only**. There is no scan for **Lessons 14–17**, so Term 4 (Lessons 11–20) can be built straight through to Lesson 13 and then Lessons 18–20, but Weeks 4–7 of Term 4 will need those pages scanned first. The PowerPoints in `Devo ppt b2 1-20/` do cover all 20 lessons, but the decks alone have no "Way in" hook, no Memory verse box, no Bible focus questions and no teacher guidance, so they aren't a substitute for the manual. Also note: the scanned PDFs have no text layer and are sideways two-page spreads — render them to images, rotate 90°, split down the middle, then read them visually.
