...
**Scripts-coverage pass (this session):** the teacher reported that students specifically love the on-screen `roleplay`/`dictation` scripts and kept asking for one on weeks that didn't have any. Audited every week from Term 3 onward and found 12 weeks with zero `roleplay`/`dictation` blocks (Term 3 Weeks 1–8; Term 4 Weeks 1, 4, 9, 10). Added original scripts to all of them — no source manual/PowerPoint was attached this session, so these are freshly written dramatisations/whole-class readings based on the Bible passage and the existing `story` block narration already in each file, in the same spirit as the Week 3/Term 2 "no printed script existed, so it was written fresh" precedent, NOT transcribed from a manual. Every week from Term 3 onward now has at least one script; most have two:
- Term 3: Week 1 (2 — midwives defy Pharaoh; princess finds baby Moses), Week 2 (2 — burning bush; Moses' last excuse), Week 3 (2 — Let my people go; hail and a broken promise), Week 4 (2 — Passover instructions dictation; Don't be afraid), Week 5 (2 — the mountain agreement; Ten Commandments dictation), Week 6 (2 — build me a sacred tent; God's promise for the future dictation), Week 7 (2 — two reports; Caleb and Joshua's plea), Week 8 (2 — the choice dictation; God's mercy dictation). Weeks 9–10 already had scripts from a prior session and were untouched.
- Term 4: Week 1 (2 — Paul's résumé of suffering dictation; the gospel in one paragraph dictation), Week 4 (2 — interview with Robert Stanford [the term's own recurring fictional case-study character, not a real person]; old life/new life dictation), Week 9 (2 — the angel visits Mary; the angel visits Joseph), Week 10 (1 — Paul commissions Timothy). Weeks 2, 3, 5, 6, 7, 8 already had scripts and were untouched.
- **Week 10 deliberately got only one script, and it's about Paul and Timothy, not Miriam.** Week 10's lesson profiles a real, named, living person (a real assistant school chaplain) — inventing dialogue/quotes for her would misattribute words to a real person, so no roleplay was written putting words in her mouth. If a script is still wanted for Week 10, it would need to come from Miriam herself (an actual quote or interview transcript she's given), not be invented.
- No `index.html` changes were needed — `roleplay`/`dictation` rendering was already fully built. No new images were added. Ran the full four-term sanity check (parse + image-key resolution) after the edits; everything loads cleanly, 40/40 weeks, no dangling image references.
- Note: `data_term3.js` Week 3's days happen to use a different (pretty-printed, quoted-key) JSON sub-style from the rest of the file, left over from an earlier session — harmless (still valid JS), just cosmetically inconsistent if you ever hand-edit that week.
